...
def get_count_matrix(args, db, db_opts):
    ...
    step=100
    batches = [doc_ids[i:i + step] for i in range(0, 1000, step)]

    _count = partial(count, args.ngram, args.hash_size)
    #_count_freq = partial(count_freq, args.ngram, args.hash_size)
...
def count_other(db,db_opts):
    ...
    with db_class(**db_opts) as doc_db:
        ...
        for i, doc_id in enumerate(doc_ids):
            document=doc_db.get_doc_text(doc_id)
            tok.tokenize(retriever.utils.normalize(document)).words()
            doc_len.append(len(document))
            num_doc += len(document)
            if i==1000:
                break
    avgdl = num_doc / corpus_size
    logger.info(f'num_doc {num_doc}')
    logger.info(f'corpus size {corpus_size}')
    logger.info(f'avgdl {avgdl}')

    logger.info(f'doc_len {doc_len}')
    return  doc_len, corpus_size, avgdl

def get_idf_matrix(cnts):
...
def get_doc_freqs(cnts):
    ...
    logging.info("freqs")
    logging.info(freqs[0])
    logging.info("end freqs")
...
if __name__ == '__main__':
    ...
    idf = get_idf_matrix(count_matrix)

    logger.info('Getting word-doc frequencies...')
    freqs = get_doc_freqs(count_matrix)
    logger.info('Getting rest of the counts...')

    doc_len, corpus_size, avgdl=count_other('sqlite', {'db_path': args.db_path})
    basename = os.path.splitext(os.path.basename(args.db_path))[0]
    basename += ('-bm25_sparse-ngram=%d-hash=%d-tokenizer=%s' %
    ...
    retriever.utils.save_sparse_csr(filename, idf, metadata)
